...
usersRouter.post('/sign-up', async (request, response, next) => {
    ...
    if (
        !/^((?=\S*?[A-Z])(?=\S*?[a-z])(?=\S*?[0-9])(?=\S*?[!@#$%^&*()-_+]).{8,})\S$/g.test(
            password
        )
    ) {
        return next({
            name: 'ValidationError',
            message:
                'Password should be at least 8 characters long and have at least 1 lowercase ,1 uppercase letter, one special character, and one number',
        });
    }
    ...
    if (process.env.NODE_ENV === 'test') {
        ...
        return response
            .status(201)
            .json({ data: 'User successfully created.' });
    }
});
